 ...
 */
export async function updateListing(listingId, userId, newQuantity, newPricePerUnit) {
  try {
    // Get listing
    const { data: listing, error: listingError } = await supabase
      .from('market_listings')
    ...
    // Get current resource amount and check if we have enough resources
    const { data: currentUser } = await supabase
      .from('users')
      .select(listing.resource_type)
      .eq('id', user.id)
    ...
    const quantityDifference = newQuantity - listing.quantity;
    const availableResources = (currentUser[listing.resource_type] || 0);

    // If quantity increased, check if user has enough resources
    if (quantityDifference > 0 && availableResources < quantityDifference) {
      throw new Error('Not enough resources to increase listing quantity');
    }

    // Update resource amount
    const updateResourceData = {};
    updateResourceData[listing.resource_type] = availableResources - quantityDifference;
    ...
    if (updateError) {
      // Revert resource update if listing update fails
      updateResourceData[listing.resource_type] = availableResources;
      await supabase.from('users').update(updateResourceData).eq('id', user.id);
