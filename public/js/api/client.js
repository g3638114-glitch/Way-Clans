...
export const apiClient = {
  ...
  async getWarriors(userId) {
    const response = await fetch(`/api/user/${userId}/warriors`);
    if (!response.ok) {
  ...
  // Get warriors of a specific type
  async getWarriorsByType(userId, type) {
    const response = await fetch(`/api/user/${userId}/warriors/${type}`);
    if (!response.ok) {
      throw new Error('Failed to load warriors');
    }
    return response.json();
  ...
  // Hire a new warrior
  async hireWarrior(userId, { type, level }) {
    const response = await fetch(`/api/user/${userId}/warriors/hire`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  // Upgrade warriors
  async upgradeWarrior(userId, { type, fromLevel, toLevel }) {
    const response = await fetch(`/api/user/${userId}/warriors/upgrade`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ type, fromLevel, toLevel }),
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  // Get warriors summary
  async getWarriorsSummary(userId) {
    const response = await fetch(`/api/user/${userId}/warriors/summary`);
    if (!response.ok) {
      throw new Error('Failed to load warriors summary');
    }
    return response.json();
  },
};
